...
def main():
    ...
    for repo in config.get("repositories", []):
        ...
        cfg.append(f"server {{  # HTTP to HTTPS redirect")
        cfg.append(f"    listen 80;")
        cfg.append(f"    listen [::]:80;")
        cfg.append(f"    server_name {repo['domain']};")
        cfg.append(f"    return 301 https://$host$request_uri;")
        cfg.append(f"}}")
        cfg.append("")  # Add a blank line for readability
        
        cfg.append(f"server {{  # HTTPS")
        cfg.append(f"    listen 443 ssl http2;")
        cfg.append(f"    listen [::]:443 ssl http2;")
        cfg.append(f"    server_name {repo['domain']};")
        cfg.append(f"    ssl_certificate /etc/letsencrypt/live/{repo['domain']}/fullchain.pem;")
        cfg.append(f"    ssl_certificate_key /etc/letsencrypt/live/{repo['domain']}/privkey.pem;")
        ...
        cfg.append(f"        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;")
